...
=======================================================================-->
	...
	<xsl:param name="serverURL">https://www.mchenry.edu</xsl:param>
	<xsl:param name="pixelsPerInch">72</xsl:param>
	<!-- Must be true() or false() [no quotes] -->
	<xsl:param name="fitToPageWidth" select="true()"/>
	<!-- Table of Contents -->
	<!-- <xsl:variable name="discoveredMeta"><xsl:for-each select="/html/head/meta | /html:html/html:head/html:meta"><xsl:for-each select="@* | @html:*">
                    @<xsl:value-of select="name()"/> = <xsl:value-of select="."/><xsl:if test="position() != last()">, </xsl:if></xsl:for-each><xsl:if test="position() != last()">;
   ...
   =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-->
	...
	<xsl:attribute-set name="body">
		<xsl:attribute name="font-size">
			<xsl:call-template name="pxTopt">
				<xsl:with-param name="px" select="12"/>
			</xsl:call-template>
		</xsl:attribute>
...
24pt</xsl:attribute>
   ...
   =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-->
	...
	<xsl:attribute-set name="table">
		<!--
     <xsl:attribute name="border-collapse">separate</xsl:attribute><xsl:attribute name="border-spacing"><xsl:call-template name="pxTopt"><xsl:with-param name="px" select="2"/></xsl:call-template></xsl:attribute><xsl:attribute name="border"><xsl:call-template name="pxTopt"><xsl:with-param name="px" select="1"/></xsl:call-template></xsl:attribute><xsl:attribute name="border-style">outset</xsl:attribute>
     -->
	</xsl:attribute-set>
   ...
   =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-->
	...
	<xsl:attribute-set name="abbr">
		<!-- e.g.,
     <xsl:attribute name="font-variant">small-caps</xsl:attribute><xsl:attribute name="letter-spacing">0.1em</xsl:attribute>
     -->
	</xsl:attribute-set>
   ...
   =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-->
	...
	<xsl:attribute-set name="img-link">
		<xsl:attribute name="border">
			<xsl:call-template name="pxTopt">
				<xsl:with-param name="px" select="2"/>
			</xsl:call-template> solid
		</xsl:attribute>
	</xsl:attribute-set>
	<!--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        Link
...
=======================================================================-->
	...
	<xsl:template match="body | html:body">
		<fo:page-sequence master-reference="all-pages">
			<fo:static-content flow-name="page-header">
				<fo:block space-before="{$page-header-margin}" space-before.conditionality="retain" text-align="center" xsl:use-attribute-sets="h1">
					<xsl:if test="$title-print-in-header = 'true'">
						<xsl:value-of select="/html/body/header"/>
					</xsl:if>
				</fo:block>
			</fo:static-content>
			...
			<fo:flow flow-name="xsl-region-body">
				<xsl:if test="$includeTocPrinted and (.//*[translate(local-name(),$tocHeadingLevels,'#######') = 'h#'] | .//html:*[translate(local-name(),$tocHeadingLevels,'#######') = 'h#'])">
					<fo:block font-weight="bold" space-after="14pt" space-before="14pt" text-align="center" xsl:use-attribute-sets="h2">
						<xsl:choose>
							<xsl:when test="string-length($tocTitle) &gt; 0">
								<fo:block space-before="0" space-before.conditionality="retain" text-align="center" xsl:use-attribute-sets="h1">
									<xsl:value-of select="/html/body/header"/>
								</fo:block>
								<fo:inline>
   ...
   =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-->
	...
	<xsl:template name="process-common-attributes">
		<!--
     <xsl:attribute name="role"><xsl:value-of select="local-name()"></xsl:value-of></xsl:attribute><xsl:choose><xsl:when test="@xml:lang"><xsl:attribute name="xml:lang"><xsl:value-of select="@xml:lang"></xsl:value-of></xsl:attribute></xsl:when><xsl:when test="@lang"><xsl:attribute name="xml:lang"><xsl:value-of select="@lang"></xsl:value-of></xsl:attribute></xsl:when></xsl:choose>
  -->
		<xsl:choose>
			<xsl:when test="@id">
   ...
   =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-->
	<xsl:template match="table | html:table">
		...
		<!--
     <fo:table-and-caption xsl:use-attribute-sets="table-and-caption"><xsl:call-template name="make-table-caption"></xsl:call-template><fo:table xsl:use-attribute-sets="table"><xsl:call-template name="process-table"></xsl:call-template></fo:table></fo:table-and-caption>
-->
	</xsl:template>
		...
		<xsl:if test="@border or @frame">
			<xsl:choose>
				<xsl:when test="@border &gt; 0">
					<xsl:attribute name="border">
						<xsl:call-template name="pxTopt">
							<xsl:with-param name="px" select="@border"/>
						</xsl:call-template>
					</xsl:attribute>
				</xsl:when>
			</xsl:choose>
...
hidden</xsl:attribute>
				...
				<xsl:otherwise>
					...
					<!--Bob changed this from outset-->
				</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
		<xsl:if test="@cellspacing">
			<!-- removed by Bob
       <xsl:attribute name="border-spacing"><xsl:call-template name="pxTopt"><xsl:with-param name="px" select="@cellspacing"/></xsl:call-template></xsl:attribute><xsl:attribute name="border-collapse">separate</xsl:attribute>
       -->
		</xsl:if>
			...
			<xsl:if test="@cellpadding">
				<xsl:attribute name="padding">
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:call-template name="pxTopt">
								<xsl:with-param name="px" select="@cellpadding"/>
							</xsl:call-template>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:attribute>
	...
	<xsl:template name="process-cell-align">
		...
		<!--
     <xsl:if test="$align"><xsl:attribute name="text-align"><xsl:choose><xsl:when test="$align = 'char'"><xsl:choose><xsl:when test="$align/../@char"><xsl:value-of select="$align/../@char"></xsl:value-of></xsl:when><xsl:otherwise><xsl:value-of select="'.'"></xsl:value-of></xsl:otherwise></xsl:choose></xsl:when><xsl:otherwise><xsl:value-of select="$align"></xsl:value-of></xsl:otherwise></xsl:choose></xsl:attribute></xsl:if>
-->
	</xsl:template>
   ...
   =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-->
	...
	<xsl:template name="process-img">
		<xsl:attribute name="src">
			<xsl:text>url('</xsl:text>[system-asset:embedded-image]
			<xsl:value-of select="@src"/>[/system-asset:embedded-image]
			<xsl:text>')</xsl:text>
		</xsl:attribute>
		<xsl:if test="@alt">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:attribute name="content-height">
						<xsl:call-template name="pxTopt">
							<xsl:with-param name="px" select="@height"/>
						</xsl:call-template>
					</xsl:attribute>
				</xsl:otherwise>
			</xsl:choose>
	...
	<xsl:template name="process-a-link">
		...
		<xsl:choose>
			<xsl:when test="@href='#'">
				...
				<xsl:attribute name="internal-destination">fo-top</xsl:attribute>
				<!-- fo-top defined above in template match="body" -->
			</xsl:when>
			...
			<xsl:when test="starts-with(@href, '/')">
				<!-- The link is internal to the CMS -->
				<xsl:choose>
					<xsl:when test="contains(@href, '#')">
						<!-- If it contains a #, we may need to rewrite the extension -->
						<xsl:choose>
                      ...
                      and we can use that for the link with just the beginning of the URL rewritten -->
							...
							<xsl:otherwise>
								...
								<xsl:attribute name="external-destination">
									<xsl:value-of select="$serverURL"/>
									<xsl:value-of select="substring-after(substring-before(@href, '#'), '/')"/>.html#
									<xsl:value-of select="substring-after(@href, '#')"/>
								</xsl:attribute>
							</xsl:otherwise>
						</xsl:choose>
					...
					<xsl:otherwise>
						<!-- Meaning - the href does not contain a # -->
						<xsl:choose>
							<xsl:when test="substring(@href, string-length(@href) - 3, 1) = '.' or substring(@href, string-length(@href) - 4, 1) = '.'">
			...
			<xsl:otherwise>
				<xsl:attribute name="external-destination">url('
