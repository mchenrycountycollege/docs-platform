...
var PROFILES_JSON_URL = "/_data/profile-data.json";
var ISCCMS = false;
if( window.location.href.indexOf( "mchenry.cascadecms.com" ) > -1 ) {
	PROFILES_JSON_URL = "/render/page.act?id=c862049cac1e009668c48ff64b25c2fe";
  ISCCMS = true;
} else if( window.location.href.indexOf( "localhost" ) > -1 ) {
	PROFILES_JSON_URL = "data.json";
} else if( window.location.href.indexOf( "dev.cascadefactory" ) > -1 ) {
	PROFILES_JSON_URL = "data.json";
}


var ACTIVE_LETTERS = [];
var prevFilterSelect = "";
$(document).ready(function() {
	...
	var lastBuiltHash = "";
      ...
      var hashDetailId = "";
        ...
        hashTags = hash;
          ...
          if( tmp[ i ].substring( 0 , 6 ) == "detail" ) {// &&  ! isNaN( tmp[ i ].substring( 6 ) ) ) {
            hashDetailId = tmp[ i ].substring( 6 );
          } else if( ! isNaN( tmp[ i ] ) ) {
      ...
      if( hashTagsArr.length > 0 ) {
        /*toggle on filters tab*/
			...
			return { start: hashStartTime , end: hashEndTime, tags: hashTags , tagsArr: hashTagsArr , search: hashSearch , pageNum: hashPageNum , detailId : hashDetailId };
		}
    var profileDetailTemplate = false;
	...
	if( $( ".entity_list_items, .detail-container" ).length > 0 ) {
	$.getJSON( PROFILES_JSON_URL , function( data ) {
		var tmpImgPattern = "";
		allData = cleanData( data );
		$( ".entity_list_items, .detail-container" ).each( function( index ) {
       
			var $listEl = $( this );//.find( ".entity_list_items" );
			...
			var $filterEl = $( '.alpha_form' );
			var $filterInputs = $( '.alpha_button' );
			var $filterSelect = $( 'select.alpha_select , .profiles_filter_select' );
			var $filterOnlySelect = $( '.profiles_filter_select' );
			var $alphaSelect = $( 'select.alpha_select' );
		
			if( $filterEl.length > 0 ) {
				showFilters( allTags , $filterOnlySelect );
				$filterSelect.change( function( e ) {
          var tmpVal = cleanFilter( e.target.value );
          hashParams.tagsArr = [];
          if( tmpVal && tmpVal != "all" ) {
          ...
          hashParams.pageNum = 0;
				...
				$( "form.filter_search" ).submit( function( e ) {
					e.preventDefault();
          onRunSearch();
        });
				$( "#filter_search_input" ).change( function( e ) {
					e.preventDefault();
          onRunSearch();
        });
        function onRunSearch() {
					var searchTerm = $( "#filter_search_input").val();//.toLowerCase();
					if( ( searchTerm.length > 1 && searchTerm != prevSearchTerm ) || ( prevSearchTerm != "" && searchTerm == "" ) ) {
            hashParams.pageNum = 0;
            hashParams.tagsArr = [];
						prevSearchTerm = searchTerm;
            hashChangedBySearch = window.top.location.hash.substring(1) != buildHash();
            window.top.location.hash = buildHash();
            setFilters( hashParams.tagsArr , $filterInputs , $filterSelect );
						activeData = searchItems( searchTerm );
          ...
          if( hashChangedBySearch ) {
            ...
            if( hashParams.detailId !== "" ) {
              showDetail( hashParams.detailId );
            } else {
      ...
      if( ! ISCCMS && window.location.href.indexOf( "?pid=" ) > -1 ) {
        showDetail( window.location.href.substring( window.location.href.indexOf( "?pid=" ) + 5 ) );
      } else if( ISCCMS && window.location.href.indexOf( "&pid=" ) > -1 ) {
        showDetail( window.location.href.substring( window.location.href.indexOf( "&pid=" ) + 5 ) );
      } else if( ISCCMS && window.top.location.href.indexOf( "&pid=" ) > -1 ) {
        showDetail( window.top.location.href.substring( window.top.location.href.indexOf( "&pid=" ) + 5 ) );
      } else if( hashParams.detailId !== "" ) {
        showDetail( hashParams.detailId );
      } else {
  ...
  function showDetail( id ) {
    if( ! profileDetailTemplate ) profileDetailTemplate = _.template( $( "#profile-detail-template" ).html() );
    var tmp = allData.find( function( i ) { return i.un == id; } );
    if( tmp ) {
      $( ".detail-container" ).html( profileDetailTemplate( tmp ) );
    ...
    return $.trim( str.toLowerCase().replace( /[^a-z]/g , '' ) );
  }
	function cleanData( allData ) {
		...
		for( var i = 0; i < allData.length; i++ ) {
			...
			if( dupIndex == -1 ) {
				...
				allData[ i ].firstLetter = allData[ i ].lname ? allData[ i ].lname.charAt( 0 ).toLowerCase() : "";
				//allData[ i ].firstLetter = allData[ i ].title ? allData[ i ].title.charAt( 0 ).toLowerCase() : "";
        if( ACTIVE_LETTERS.indexOf( allData[ i ].firstLetter ) == -1 ) {
          ACTIVE_LETTERS.push( allData[ i ].firstLetter );
        }
        
        if( allTags.indexOf( allData[ i ].dept ) == -1 ) {
          allTags = [ ...allTags , allData[ i ].dept ];
        }
        allData[ i ].searchString = allData[ i ].title + ' ' + allData[ i ].fname + ' ' + allData[ i ].lname + ' ' + allData[ i ].dept + ' ' + allData[ i ].phone + ' ' + allData[ i ].sections.map( function(x){return x.name + " " + x.title;}).join( " " ) + ' ' + allData[ i ].degrees.map( function(x){return x.institution + " " + x.degree;}).join( " " );
        
        // + ' ' + allData[ i ].department.join( " " ) + ' ' + allData[ i ].phoneNumbers.map( function( x ) { return x[ 1 ]; } ).join( " " ) + ' ' + allData[ i ].emailAddresses.map( function( x ) { return x[ 1 ]; } ).join( " " ) + ' ' + allData[ i ].locations.map( function( x ) { return x[ 1 ]; } ).join( " " );
				//allData[ i ].tags = allData[ i ].tags.split( "|" );
				...
				//allData[ i ].imageAlt = allData[ i ].title;
				//tmpImgPattern = allData[ i ].link.replace( "https://news.emory.edu//stories/" , "" );
				//tmpImgPattern = tmpImgPattern.replace( "/index.html" , "" );
				//allData[ i ].image = "";//"http://news.emory.edu/stories/" + tmpImgPattern + "/thumbs/feed.jpg";
      ...
      return a.lname == b.lname ? ( a.fname > b.fname ? 1 : -1 ) : ( a.lname > b.lname ? 1 : -1 );
    } );
		return allData;
	}
	...
	function setFilters( tagArr , $filterInputs , $filterSelect , isFirstLoad ) {
		...
		$filterSelect.each( function( idx , tmpEle ) {
      $( tmpEle ).val( "all" );
      $( tmpEle ).find( "option" ).each( function( idx ) {
        if( tagArr.indexOf( cleanFilter( $(this).val() ) ) > -1 ) {
          if( isFirstLoad ) prevFilterSelect = cleanFilter( $( this ).val() );
          $(tmpEle).val( $(this).val() );
          return false;
        }
      })
    ...
    setFilters( tagArr , $filterInputs , $filterSelect , isFirstLoad );
		...
		if( tagArr.length > 0 ) {// || endDate != "" || startDate != "" ) {
			ret = allData.filter( function( item ) {
				var keep = tagArr.length == 0;
        if( tagArr[ 0 ].length == 1 ) {
          keep = item.firstLetter == tagArr[ 0 ];
        } else {
          keep = cleanFilter( item.dept ) == tagArr[ 0 ];
        }
				return keep;
			} );
			//console.log( ret );
  ...
  function updatePagination( items ) {
    ...
    $pageSelect.off( 'change' );
    $( "form.pagination_form" ).on( 'submit' , runPagination);
    $pageSelect.on( 'change' , runPagination);
  ...
  function updatePaginationArrows( pageTotal , $pageSelect ) {
	...
	function showFilters( data , $el ) {
		$el.append( '<option id="optionAll" value="' + 'all' + '" >' + 'Select a Department' + '</option>' );
		for( var i = 0; i < data.length; i++ ) {
			if( data[ i ] != "" ) {
				$el.append( '<option id="option' + i + '" value="' + data[ i ] + '" >' + data[ i ] + '</option>' );
			}
		}
	}
  ...
  function appendCollecitonItem( itemData , $el ) {
      ...
      //var templateFirst = _.template( $( "#program-teaser-template" ).html().replace( 'class="item"' , 'class="item first"' ) );
      //$el.empty();
